...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { title: dict.nav.contact, description: dict.contact.metaDescription };
}
        ...
        <Container className="grid grid-cols-1 gap-16 lg:grid-cols-[1fr_0.8fr]">
          <Reveal>
            <div className="rounded-3xl border border-mist-300 bg-white p-8 sm:p-10 dark:border-ink-700 dark:bg-ink-800">
              <h2 className="font-display text-xl font-semibold text-navy-900 dark:text-white">{t.quickInquiryTitle}</h2>
              <p className="mt-2 text-sm text-charcoal-600 dark:text-white/60">{t.quickInquiryDesc}</p>
              <div className="mt-8">
          ...
          <Reveal delay={0.1} className="space-y-6">
            <div className="rounded-2xl border border-mist-300 bg-mist-50 p-7 dark:border-ink-700 dark:bg-ink-800">
              <div className="text-xs font-semibold uppercase tracking-[0.14em] text-emerald-600 dark:text-emerald-400">{t.germanyHQ}</div>
              <div className="mt-4 space-y-3 text-sm text-charcoal-700 dark:text-white/70">
                <div className="flex items-start gap-3">
                  ...
                  <span>
                    ...
                    <br />
                    {company.address.postal} {company.address.city}, {countryNames[rawLocale]}
                  </span>
                </div>
                ...
                <div className="text-xs text-charcoal-400 dark:text-white/45">
                  {t.contactLabel}: {company.contactPerson}
                </div>
              </div>
            </div>

            <div className="rounded-2xl border border-mist-300 bg-mist-50 p-7 dark:border-ink-700 dark:bg-ink-800">
              <div className="text-xs font-semibold uppercase tracking-[0.14em] text-emerald-600 dark:text-emerald-400">{t.brazilPartner}</div>
              <div className="mt-4 space-y-3 text-sm text-charcoal-700 dark:text-white/70">
                ...
                >
                  <MessageCircle size={16} /> {t.messageWhatsapp}
                </a>
              </div>
            </div>
