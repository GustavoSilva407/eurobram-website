...
export function InsightsPreview({ locale, dict }: { locale: Locale; dict: Dictionary }) {
  const t = dict.home.insightsPreview;
  return (
      ...
      <Container>
        <div className="flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-end">
          ...
          <Reveal delay={0.1}>
            ...
            >
              {t.allInsights} <ArrowUpRight size={15} />
            </Link>
          </Reveal>
        </div>

        <RevealGroup className="mt-14 grid grid-cols-1 gap-6 lg:grid-cols-3">
          {dict.insights.posts.slice(0, 3).map((post) => (
            <RevealItem key={post.slug}>
              <Link
                href={href(locale, `/insights/${post.slug}`)}
